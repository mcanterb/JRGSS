...
public class Font extends RubyObject {
                ...
                if (listOfFile.isFile()) {
                    try {
                        ...
                        if (OS.CURRENT_OS == OS.WINDOWS && SystemArchitecture.IS_64BIT) {
                            font = SystemArchitecture.getStringForAddress(SystemArchitecture.getPtr(address + 6L * NativeLong.SIZE), bb);
                        } else {
                            font = SystemArchitecture.getStringForAddress(SystemArchitecture.getPtr(address + 5L * NativeLong.SIZE), bb);
                        }
    ...
    public boolean isOutline() {
        return this.outline && this.outColor.getAlpha() != 0;
    }

    private void update() {
        String fontName = this.name.first().asJavaString();
        this.loadedResolution = Graphics.getHiResScale();
        if (fontCache.containsKey(new Font.FontCacheKey(this.size, fontName, this.bold))) {
            ...
            JRGSSGame.runWithGLContext(() -> {
                ...
                param.size = (int) (this.size * 0.85 * Graphics.getHiResScale());
                param.flip = true;
                param.genMipMaps = true;
                param.borderColor = com.badlogic.gdx.graphics.Color.WHITE;
                param.borderWidth = this.bold ? 0.2F * Graphics.getHiResScale() : 0.0F;
                param.gamma = 1.0F;
                ...
                param.magFilter = TextureFilter.Linear;
                param.minFilter = TextureFilter.Linear;
                param.borderWidth = (this.bold ? 1.2F : 1.0F) * Graphics.getHiResScale();
    ...
    public BitmapFont getOutlineFont() {
        if (Math.abs(this.loadedResolution - Graphics.getHiResScale()) > 0.001 || this.outlineFont.get() == null) {
            this.update();
        }
    ...
    public BitmapFont getBitmapFont() {
        if (Math.abs(this.loadedResolution - Graphics.getHiResScale()) > 0.001 || this.bitmapFont.get() == null) {
        ...
        public boolean equals(Object o) {
            if (o == this) {
                ...
                if (!other.canEqual(this)) {
                    ...
                    return Objects.equals(this$name, other$name) && this.isBold() == other.isBold();
                }
            }
        }
