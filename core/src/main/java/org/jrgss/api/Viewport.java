...
public class Viewport extends AbstractRenderable {
    ...
    public void render(JrgssBatch batch) {
        ...
        if (!this.tone.isZero()) {
            ...
            batch.flush();
            Graphics.tempBufferEnd();
            AlphaBlendingShader.setBlendColor(this.defaultBlend, batch);
            tempBuffer.get().begin();
            AlphaBlendingShader.withToneChange(this.tone, batch, () -> {
                Gdx.gl20.glClear(16384);
                batch.setColor(1.0F, 1.0F, 1.0F, 1.0F);
                batch.draw(Graphics.getTempBuffer().getColorBufferTexture(), 0.0F, 0.0F, (float) Graphics.getWidth(), (float) Graphics.getHeight());
            });
            tempBuffer.get().end();
            Graphics.tempBufferBegin();
            Gdx.gl20.glClear(16384);
            batch.setColor(1.0F, 1.0F, 1.0F, 1.0F);
            AlphaBlendingShader.setBlendColor(this.defaultBlend, batch);
